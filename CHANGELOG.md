...
## [1.8.7] — Клик по строке «Ошибок клиента» в карточке
- Из карточки пользователя убрана отдельная кнопка; строка «Ошибок клиента»
  теперь сама кликабельна (внешний вид не меняли) и ведёт на страницу ошибок
  пользователя.
