...
**Текущая: 1.12.7.** Следующая фикс → 1.12.8 (далее — реальные LLM/vision адаптеры либо v1.13.0 Честный ЗНАК).
...
- v1.12.0 ✅ **Product Extraction foundation**: таблица `product_extraction_jobs` (типы TEXT/IMAGE_FALLBACK, статусы 0–5), `ExtractionEligibilityPolicy` (PHOTO_UNREADABLE/ERROR→image fallback; текст≥100 & conf≥0.35→text), создание задачи после OCR. OCR (сырой текст) и Extraction (понимание) разделены.
  ...
  - v1.12.6 ✅ Client extraction UX: `extraction[]` в `/me/scans/{barcode}` + блок «Данные о продукте» (показ QUEUED/IN_PROGRESS/STRUCTURED/NEEDS_REVIEW, SKIPPED/FAILED скрыты как шум заглушки; polling).
  - v1.12.7 ✅ Extraction observability: Prometheus `product_extraction_jobs{status,code}` + queue_size/oldest_age (ExtractionMetrics по образцу OcrMetrics).
...
## Статус веток
staging(test)=1.12.7 (extraction observability — Prometheus) · stable(main)=1.8.0 · production(release)=без изменений.
