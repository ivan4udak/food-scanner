...
**Текущая: 1.12.6.** Следующая фикс → 1.12.7 (далее — реальные LLM/vision адаптеры либо v1.13.0 Честный ЗНАК).
...
## Статус веток
staging(test)=1.12.6 (client extraction UX в «Мои сканы») · stable(main)=1.8.0 · production(release)=без изменений.
