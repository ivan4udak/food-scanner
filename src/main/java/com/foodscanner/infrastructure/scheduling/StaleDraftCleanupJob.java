...
public class StaleDraftCleanupJob {
    ...
    public StaleDraftCleanupJob(PurgeStaleDraftsUseCase purge,
    ...
    public void cleanup() {
        ...
        if (r.draftsDeleted() > 0 || r.objectsDeleted() > 0) {
            log.info("Очистка мусора: удалено черновиков={}, объектов хранилища={}",
                r.draftsDeleted(), r.objectsDeleted());
        }
    }
}
